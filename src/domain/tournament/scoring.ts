...
function scoreOrDefault(raw: unknown, fallback: number | null): number | null {
...
export function getUnitScoreForGame(
  ...
  if (!teamSize) {
    return scoreOrDefault(
      state.scores[`r${roundIndex}-rm${room}-p${position}${gamePart}`],
      fallback,
    );
  }
  ...
  const team = roomAssignments[position]
    ? buildTeamMap(state)[roomAssignments[position].name]
    : undefined;
  const values: number[] = [];
  ...
  for (let memberIndex = 0; memberIndex < teamSize; memberIndex += 1) {
    ...
    let value = scoreOrDefault(
      state.scores[
        `r${roundIndex}-rm${room}-p${position}${gamePart}-m${memberIndex}`
      ],
      null,
    );
    if (value === null) {
  ...
  return computeTeamScore(
    values,
    rule,
    getDefenderIndex(state, team?.teamId, roundIndex),
  );
}
...
export function getFinalUnitScore(
  ...
  for (let memberIndex = 0; memberIndex < teamSize; memberIndex += 1) {
    ...
    const value = scoreOrDefault(
      state.finalScores[`${gamePart}-${key}-m${memberIndex}`],
      null,
    );
    values.push(value ?? 0);
  ...
  return computeTeamScore(
    values,
    rule,
    getDefenderIndex(state, key, finalRoundIndex),
  );
}
...
export function scoreKeysForPosition(options: {
  ...
  for (let game = 1; game <= numGames; game += 1) {
    const base =
      `r${roundIndex}-rm${room}-p${position}` +
      (numGames > 1 ? `-g${game}` : "");
    if (teamSize) {
...
export function groupByScore<T extends { score: number }>(
  scoredDescending: T[],
): T[][] {
  const clusters: T[][] = [];
  let index = 0;
...
export function tieResolutionList(
