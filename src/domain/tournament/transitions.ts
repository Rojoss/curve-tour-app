...
export const PENDING_TIES_MESSAGE =
  "Resolve all tie-breaks before advancing.";

export type RoundAdvanceBlockReason =
  | "pending-ties"
  | "malformed-final"
  | "invalid-room-split";

export type RoundAdvanceNoopReason = "last-round" | "grand-final";
...
function cloneForTransition(state: TournamentState): TournamentState {
  return {
    ...
    assignments: state.assignments.map((round) =>
      round.map((assignment) => ({ ...assignment })),
    ),
...
export function reserveWindowAtCurrentRound(
  state: Pick<TournamentState, "cfg" | "rounds" | "curRound">,
): boolean {
  let cutRound = -1;
  if (state.cfg.poolingPhase === "group-stage") {
    ...
    cutRound = state.rounds.findIndex(
      (round) => !round.isNoElim && !round.isFinal,
    );
  }
...
function finalizeDoubleEliminationRound(
  ...
  setArrayIndex(state.byes, roundIndex, byeNames);
  for (const name of byeNames) {
    assignments.push({ name, room: null, isLucky: false });
  }
  setArrayIndex(state.assignments, roundIndex, assignments);

  if (targetRound.bracket === "grand-final") {
    const sourceIndex = state.rounds.findIndex(
      (round) =>
        round.bracket === "winners" && round.winnersTo === roundIndex,
    );
    if (sourceIndex !== -1) {
      const finalist = doubleEliminationComputeAdvancement(
        state,
        sourceIndex,
      ).winners[0]?.name;
...
function doubleEliminationFinalMessage(
  actual: number,
  expected: number,
): string {
...
function advanceDoubleElimination(
  ...
  if (state.byes[roundIndex]?.length) {
    result.winners = [
      ...state.byes[roundIndex].map((name) => ({ name })),
      ...result.winners,
    ];
  }
  ...
  const pendingWinners =
    winnersTarget !== null && winnersTarget !== undefined
      ? [
      ...
      : null;
  const pendingLosers =
    losersTarget !== null && losersTarget !== undefined
      ? [
  ...
  if (nextRound.isFinal && poolAtNext.length !== nextRound.players) {
    return {
      ...
      message: doubleEliminationFinalMessage(
        poolAtNext.length,
        nextRound.players,
      ),
      state: input,
  ...
  if (winnersTarget !== null && winnersTarget !== undefined) {
    state.pendingBracketSeeds[winnersTarget] = pendingWinners ?? [];
  }
  if (losersTarget !== null && losersTarget !== undefined) {
    state.pendingBracketSeeds[losersTarget] = pendingLosers ?? [];
  }
  if (
    winnersTarget !== null &&
    winnersTarget !== undefined &&
    result.luckyNames.length
  ) {
    setArrayIndex(state.luckyLosers, winnersTarget, [
      ...(state.luckyLosers[winnersTarget] ?? []),
      ...result.luckyNames,
    ]);
  }
...
export function advanceTournamentRound(
  ...
  if (result.luckyNames !== null) {
    setArrayIndex(state.luckyLosers, roundIndex + 1, result.luckyNames);
  }
  ...
  if (nextRound.isGroupStage) {
    ...
    setArrayIndex(state.byes, roundIndex + 1, [
      ...(nextRound.groupByes ?? []),
    ]);
    ...
    if (state.gamemodeConfig.oddCountStrategy === "bye") {
      ...
      if (nextRound.bracketPhaseFirstRound) {
        needed =
          nextPowerOf2AndRounds(advancing.length).bracketSize -
          advancing.length;
      ...
      if (needed > 0) {
        ...
        if (nextRound.isNoElim && needed === 1) {
          const chosen = selectPoolingBye(
            advancing,
            state.poolingByeCounts,
          );
          if (chosen) {
            newByes.push(chosen);
            advancing = advancing.filter((entry) => entry !== chosen);
            state.poolingByeCounts[chosen.name] =
              (state.poolingByeCounts[chosen.name] ?? 0) + 1;
          }
        } else {
  ...
  state.curRound += 1;
  setArrayIndex(state.assignments, state.curRound, seeded);
  state.reserveOpen = reserveWindowAtCurrentRound(state);
  state.needsSave = true;
  return { status: "advanced", state };
}
