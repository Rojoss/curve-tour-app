import { computeRankings, type TournamentState } from "../../domain/tournament";
...
export async function buildRankingsImage(state: TournamentState): Promise<Blob | null> {
  ...
  if (!data) return null;
  const rows = [
    ...data.stillActive.map((entry) => ({ rank: "—", name: entry.label, badge: "STILL IN TOURNAMENT", champion: false })),
    ...data.finalists.map((entry) => ({ rank: String(entry.rank), name: entry.label, badge: "REACHED FINAL", champion: entry.rank === 1 })),
    ...data.eliminatedList.map((entry) => ({ rank: String(entry.rank), name: entry.label, badge: entry.round.isFinal ? "FINAL" : entry.round.isSemis ? "SEMIS" : `ROUND ${entry.round.roundNum}`, champion: false })),
  ];
  const width = 900;
  const rowHeight = 38;
