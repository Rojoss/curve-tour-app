    ...
    <div id="panel-setup">
      ...
      <TwoColumnGrid>
        <Panel>
          ...
          {!isRace ? <Field label="Finals format"><Select id="cfg-finals-games" value={setup.finalsGames} onChange={(e) => change("finalsGames", e.target.value)}>{[1,2,3,4].map((n) => <option key={n} value={n}>{n === 1 ? "Single game" : `${n} games (sum)`}</option>)}</Select></Field> : <Field label="Grand Final — wins needed to take the title"><div className="flex gap-2.5"><Input aria-label="Winners bracket target" type="number" min="1" value={setup.grandFinalWbTarget} onChange={(e) => change("grandFinalWbTarget", e.target.value)} /><Input aria-label="Losers bracket target" type="number" min="1" value={setup.grandFinalLbTarget} onChange={(e) => change("grandFinalLbTarget", e.target.value)} /></div></Field>}
          {isSingle ? <>
          ...
          {error ? <Alert id="generate-error" tone="danger">{error}</Alert> : null}
          <ButtonRow><Button variant="primary" onClick={generate}>▶ Generate Schedule</Button></ButtonRow>
        </Panel>
        <Panel>
          <Field label="Registered players"><FieldDisplay id="cfg-n-display">{state.confirmedCount ?? "—"}{state.reserves.length ? ` (+ ${state.reserves.length} reserves)` : ""}</FieldDisplay></Field>
          <PanelTitle hint={<>— confirmed {teamSize ? "teams" : "players"}, one per line</>}>Roster</PanelTitle>
          ...
          <PanelTitle className="mt-4" hint="— one per line">Reserves</PanelTitle>
          <Textarea id="cfg-reserves" className="min-h-15" value={setup.reserves} onChange={(e) => change("reserves", e.target.value)} />
          {teamSize ? <><PanelTitle className="mt-4" hint="— substitutes for one member">Individual reserves</PanelTitle><Textarea id="cfg-reserve-individuals" className="min-h-15" value={setup.reserveIndividuals} onChange={(e) => change("reserveIndividuals", e.target.value)} /></> : null}
          <ButtonRow><Button onClick={loadRoster}>Load roster & reserves</Button><span className="self-center text-xs text-muted">{status}</span></ButtonRow>
        </Panel>
      </TwoColumnGrid>
      {state.rounds.length && !state.started ? <Panel id="preview-wrap">
        <PanelTitle>Schedule Preview</PanelTitle>
        <Timeline>{state.rounds.map((round, index) => <TimelineItem key={index} label={round.isFinal ? "Final" : round.bracket === "winners" ? "WB" : round.bracket === "losers" ? "LB" : `${round.players} ${format?.unitLabelPlural ?? "Players"}`}>{round.roundNum}</TimelineItem>)}</Timeline>
        <ButtonRow><Button variant="success" onClick={() => updateState({ ...state, curRound: 0, started: true })}>✓ Confirm & Start</Button></ButtonRow>
      </Panel> : null}
    </div>
  );
}
